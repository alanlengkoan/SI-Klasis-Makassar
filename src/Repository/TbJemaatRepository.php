...
class TbJemaatRepository extends ServiceEntityRepository
{
    ...
    {
        $sql = "SELECT j.id_jemaat, j.nama, j.tmp_lahir, j.tgl_lahir, j.jen_kel, j.alamat, j.pekerjaan, j.no_telpon FROM App\Entity\TbJemaat j WHERE j.id_gereja = '$id' AND DATE_FORMAT(j.tgl_lahir, '%m-%d') BETWEEN DATE_FORMAT('$tgl_awal', '%m-%d') AND DATE_FORMAT('$tgl_akhir', '%m-%d')";
